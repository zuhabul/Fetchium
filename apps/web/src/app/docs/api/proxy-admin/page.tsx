...
export default function ProxyAdminApiReference() {
  return (
    <article className="docs-content max-w-3xl">
      ...
      <CodeBlock language="bash" code={`curl https://api.fetchium.com/v1/proxy/stats \\
  ...
  "proxies": [
    {
      ...
      <CodeBlock language="bash" code={`curl -X POST https://api.fetchium.com/v1/proxy/reset \\
  -H "X-Admin-Secret: $FETCHIUM_ADMIN_SECRET"

curl -X POST https://api.fetchium.com/v1/proxy/purge \\
  -H "X-Admin-Secret: $FETCHIUM_ADMIN_SECRET"`} />

      <h2>Test one proxy</h2>
      <CodeBlock language="bash" code={`curl -X POST https://api.fetchium.com/v1/proxy/test \\
  -H "X-Admin-Secret: $FETCHIUM_ADMIN_SECRET"`} />
