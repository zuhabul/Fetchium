...
pub async fn refund(
    ...
    let _ = db.log_audit(Some(&auth.user.id), Some(&auth.user.role), "org", Some(&org_id), "billing.refund", None);
...
pub async fn credit(
    ...
    db.add_credit(&org_id, body.amount_cents, body.reason.as_deref(), Some(&auth.user.id))
        .map_err(|e| (axum::http::StatusCode::BAD_REQUEST, Json(serde_json::json!({"error": e.to_string()}))))?;
    let _ = db.log_audit(Some(&auth.user.id), Some(&auth.user.role), "org", Some(&org_id), "billing.credit", None);
    Ok(Json(serde_json::json!({"ok": true})))
}
...
pub async fn webhook_replay(
    ...
    let _ = db.log_audit(Some(&auth.user.id), Some(&auth.user.role), "billing", Some(&id), "webhook.replay", None);
    Ok(Json(serde_json::json!({"ok": true})))
}
