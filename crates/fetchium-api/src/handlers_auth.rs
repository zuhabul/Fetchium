...
///
/// Reads `FETCHIUM_ADMIN_SECRET` first; falls back to `HSX_ADMIN_SECRET` (deprecated).
...
fn check_admin(headers: &HeaderMap) -> bool {
    let secret = std::env::var("FETCHIUM_ADMIN_SECRET")
        .or_else(|_| {
            ...
            if val.is_ok() {
                tracing::warn!(
                    "HSX_ADMIN_SECRET is deprecated — rename it to FETCHIUM_ADMIN_SECRET"
                );
            }
            val
        })
        .expect(
            "FETCHIUM_ADMIN_SECRET must be set (generate with: openssl rand -hex 32). \
