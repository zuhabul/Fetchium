//! `fetchium ai` — AI-powered synthesis using configured provider (Mode D, PRD §10).
...
pub async fn run(args: AiArgs, config: &HsxConfig) -> anyhow::Result<()> {
    ...
    println!("{}", style("─".repeat(60)).dim());
    let mode_tag = if result.fast_mode {
        style("snippets").yellow()
    } else {
        style("full").green()
    };
    println!(
    ...
    println!(
        "{} {}ms  {} {}ms  {} {}ms  {} {}ms",
        style("Total:").dim(),
        style(result.total_ms).bold(),
        style("search:").dim(),
        style(result.search_ms).cyan(),
        style("fetch:").dim(),
        style(result.fetch_ms).cyan(),
        style("ai:").dim(),
        style(result.ai_ms).cyan(),
    );
