...
fn search_help_does_not_crash() {
    fetchium().args(["search", "--help"]).assert().success();
}

#[test]
fn fetch_help_does_not_crash() {
    fetchium().args(["fetch", "--help"]).assert().success();
}
...
fn research_help_does_not_crash() {
    fetchium().args(["research", "--help"]).assert().success();
}

#[test]
fn compare_help_does_not_crash() {
    fetchium().args(["compare", "--help"]).assert().success();
}

#[test]
fn monitor_help_does_not_crash() {
    fetchium().args(["monitor", "--help"]).assert().success();
}

#[test]
fn index_help_does_not_crash() {
    fetchium().args(["index", "--help"]).assert().success();
}
