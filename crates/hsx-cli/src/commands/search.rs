...
use hsx_core::cache::{search_key, MemoryCache};
use hsx_core::config::HsxConfig;
use hsx_core::http::client::HttpClient;
use hsx_core::output::{format_search_json, format_search_markdown};
use hsx_core::search::orchestrator::{OrchestratorConfig, SearchOrchestrator};
use hsx_core::types::{PdsTier, ResourceTier, SearchMeta, SearchMode, SearchResult};
...
pub async fn run(
    ...
    if !args.backends.is_empty() {
        let parsed: Vec<hsx_core::types::BackendId> = args
            .backends
            .iter()
            .filter_map(|s| hsx_core::search::orchestrator::parse_backend_id(s))
            .collect();
        if !parsed.is_empty() {
    ...
    if args.trust_verify && !quiet {
        let acs = hsx_core::intelligence::acs::AdversarialContentShield::new();
