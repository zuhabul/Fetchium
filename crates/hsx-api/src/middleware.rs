...
/// Axum extractor that validates `Authorization: Bearer hsx_xxx` and injects
    ...
    fn into_response(self) -> Response {
        let (status, error_type, message) = match self {
            AuthError::MissingToken => (
                ...
                "missing_token",
                "Authorization header required: Bearer hsx_...",
            ),
    ...
    S: Send + Sync,
{
    ...
    async fn from_request_parts(parts: &mut Parts, state: &S) -> Result<Self, AuthError> {
        ...
        if !raw_key.starts_with("hsx_") || raw_key.len() < 20 {
