...
//!
//! ## Key format
//! `hsx_` + 32 lowercase hex chars (128-bit random token)
...
pub struct ApiKeyRecord {
    pub id: String,
    pub name: String,
    pub key_prefix: String, // first 8 chars of raw key for display: `hsx_xxxx`
    pub plan: String,
...
impl AuthDb {
    ...
    pub fn create_key(&self, name: &str, plan: &str) -> Result<(String, ApiKeyRecord)> {
        ...
        let token: String = (0..32)
            ...
            .collect();
        let raw_key = format!("hsx_{token}");
        let key_prefix = raw_key[..12].to_string(); // "hsx_" + 8 hex chars
...
mod tests {
    ...
    fn create_and_validate_key() {
        ...
        assert!(raw_key.starts_with("hsx_"));
        assert_eq!(raw_key.len(), 68); // "hsx_" + 64 hex (32 random bytes)
    ...
    fn invalid_key_returns_none() {
        let db = test_db();
        let result = db
            .validate_key("hsx_nonexistent00000000000000000000")
            .unwrap();
