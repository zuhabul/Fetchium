...
// ─── Unified Fetchium auth store (~/.fetchium/auth.json) ─────────────────────
//
...
impl FetchiumAuth {
    ...
    /// Return the first API key (single `Api` or first of `ApiPool`).
    ///
    /// Prefer [`api_keys`] when you need all keys for pool-based rotation.
    pub fn api_key(&self) -> Option<&str> {
        match self {
            Self::Api { key } => Some(key.as_str()),
            Self::ApiPool { keys } => keys.first().map(|k| k.as_str()),
            _ => None,
        }
    }
    ...
    ///
    /// Returns a single-element slice for `Api`, all keys for `ApiPool`,
    /// and an empty vec for `Oauth`.
    pub fn api_keys(&self) -> Vec<&str> {
        match self {
            Self::Api { key } => vec![key.as_str()],
            Self::ApiPool { keys } => keys.iter().map(|k| k.as_str()).collect(),
            Self::Oauth { .. } => vec![],
        }
    }

    /// Number of API keys stored (0 for OAuth credentials).
...
/// Add an API key to the provider's key pool in the auth store.
///
/// - If no entry exists, creates a new `ApiPool` with the single key.
