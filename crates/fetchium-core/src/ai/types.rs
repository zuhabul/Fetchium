...
impl AiConfig {
    ...
    pub fn from_fetchium_config(fetchium_config: &crate::config::HsxConfig) -> Self {
...
mod parse_tests {
    ...
    fn from_fetchium_config_reads_ollama_host() {
        let mut fetchium_config = HsxConfig::default();
...
pub struct AiPreviewResult {
    ...
    pub total_ms: u64,
    /// Time spent on search phase (ms).
    pub search_ms: u64,
    /// Time spent fetching + extracting sources (ms). 0 in fast/snippet mode.
    pub fetch_ms: u64,
    /// Time spent on AI call (ms).
    pub ai_ms: u64,
    /// Whether snippets-only fast mode was used (no full-page fetch).
    pub fast_mode: bool,
}
