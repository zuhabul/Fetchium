...
async fn call_provider(
    ...
    match kind {
        ...
        ProviderKind::Gemini => {
            // Collect full key pool from all sources (env, config, auth store).
            // Pool is tried in random order; HTTP 429 triggers automatic failover.
            let keys = collect_gemini_keys(entry);
            if !keys.is_empty() {
                call_gemini_api_key_pool(&keys, &model, messages, ai_config, streaming, on_token)
                    .await
            } else if let Some(access_token) = get_gemini_access_token_if_valid() {
                tracing::info!("Using Gemini OAuth (Gemini CLI subscription)");
                call_gemini_oauth(
                    &access_token,
                    &model,
                    messages,
                ...
                // Try to refresh the expired OAuth token
...
// ─── Gemini key pool helpers ─────────────────────────────────────────────────

/// Mask an API key for safe logging: shows first 8 and last 4 characters.
...
fn collect_gemini_keys(entry: &crate::ai::providers::ProviderEntry) -> Vec<String> {
    ...
    // 1. GEMINI_API_KEYS — comma-separated pool (highest priority for pool users)
    if let Ok(multi) = std::env::var("GEMINI_API_KEYS") {
        for k in multi.split(',') {
            push(k.trim().to_string());
        }
    }
...
async fn call_gemini_cli(
    ...
    for try_model in &models_to_try {
        ...
        // Redirect stdin to /dev/null — gemini CLI hangs waiting for interactive
        // input when stdin is inherited from a terminal or pipe.
