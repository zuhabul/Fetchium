...
/// ```no_run
/// # use std::sync::Arc;
/// # use hsx_core::search::fallback::FallbackChain;
...
mod tests {
    ...
    impl ErrorBackend {
        fn new(id: BackendId) -> (Arc<AtomicUsize>, Arc<dyn SearchBackend>) {
            let counter = Arc::new(AtomicUsize::new(0));
    ...
    async fn chain_uses_first_backend_when_sufficient_results() {
        let (counter1, b1) = MockBackend::new(BackendId::Google, 10);
        let (counter2, b2) = MockBackend::new(BackendId::Bing, 10);
    ...
    async fn chain_falls_back_when_too_few_results() {
        let (_c1, b1) = MockBackend::new(BackendId::Google, 2); // Only 2 results
        let (counter2, b2) = MockBackend::new(BackendId::Bing, 10);
    ...
    async fn chain_skips_error_backends_and_falls_back() {
        let (err_counter, b_err) = ErrorBackend::new(BackendId::Google);
        let (ok_counter, b_ok) = MockBackend::new(BackendId::Bing, 7);
    ...
    async fn chain_returns_empty_when_all_error() {
        let (_c1, b1) = ErrorBackend::new(BackendId::Google);
        let (_c2, b2) = ErrorBackend::new(BackendId::Bing);
    ...
    async fn chain_with_zero_min_results_always_accepts() {
        let (_counter, b) = MockBackend::new(BackendId::DuckDuckGo, 1); // Only 1 result
        let mut chain = FallbackChain::new();
    ...
    fn chain_len_and_is_empty() {
        ...
        let (_c, b) = MockBackend::new(BackendId::DuckDuckGo, 0);
        chain.add(b, 1);
        assert!(!chain.is_empty());
    ...
    fn build_web_search_chain_respects_priority() {
        ...
        let (_c1, b1) = MockBackend::new(BackendId::DuckDuckGo, 5);
        let (_c2, b2) = MockBackend::new(BackendId::Brave, 5);
