...
use crate::rank::signals::{
    authority_score, bm25_score, consensus_score, depth_score,
    diversity_score, evidence_score, semantic_score, temporal_score,
    ScoringContext,
};
...
impl IntentWeights {
    ...
    pub fn for_intent(intent: QueryIntent) -> Self {
        match intent {
            ...
            QueryIntent::Code => Self {
                bm25: 0.25, semantic: 0.15, temporal: 0.05, authority: 0.15,
                evidence: 0.05, diversity: 0.10, depth: 0.15, consensus: 0.10,
            },
            QueryIntent::HowTo => Self {
                bm25: 0.20, semantic: 0.15, temporal: 0.10, authority: 0.10,
                evidence: 0.10, diversity: 0.10, depth: 0.15, consensus: 0.10,
            },
            QueryIntent::Comparison => Self {
                bm25: 0.15, semantic: 0.20, temporal: 0.05, authority: 0.15,
                evidence: 0.15, diversity: 0.15, depth: 0.10, consensus: 0.05,
            },
            // Balanced fallback for Verification, Opinion, Data, and any new variants
            _ => Self {
                bm25: 0.15, semantic: 0.15, temporal: 0.10, authority: 0.15,
                evidence: 0.10, diversity: 0.10, depth: 0.10, consensus: 0.15,
            },
        }
    }
    ...
    pub fn sum(&self) -> f64 {
        self.bm25 + self.semantic + self.temporal + self.authority
...
pub fn hyperfusion_rank(
    ...
    let mut raw: Vec<RawScores> = results.iter().map(|r| {
        RawScores {
    ...
    let fusion_scores: Vec<f64> = normalized.iter().map(|s| {
        s.bm25      * weights.bm25
...
fn min_max_normalize(scores: Vec<RawScores>) -> Vec<RawScores> {
    ...
    macro_rules! normalize_field {
        ($scores:expr, $field:ident) => {{
            let min = $scores.iter().map(|s| s.$field).fold(f64::INFINITY, f64::min);
            let max = $scores.iter().map(|s| s.$field).fold(f64::NEG_INFINITY, f64::max);
            let range = max - min;
            if range < EPSILON {
                for s in $scores.iter_mut() { s.$field = 0.5; }
            } else {
...
pub fn detect_intent(query: &str) -> QueryIntent {
    ...
    if q.contains("today") || q.contains("2025") || q.contains("2026") || q.contains("latest") || q.contains("news") {
...
mod tests {
    ...
    fn weights_sum_to_one_after_normalize() {
        for intent in [
            QueryIntent::Factual, QueryIntent::CurrentEvents,
            QueryIntent::Academic, QueryIntent::Code,
    ...
    fn hyperfusion_ranks_relevant_higher() {
        ...
        hyperfusion_rank(&mut results, "Rust programming language", QueryIntent::Factual, 0.5, &HashMap::new());

        // Rust result should be ranked first
        assert_eq!(results[0].url, "https://rust-lang.org", "Rust should rank first");
    ...
    fn hyperfusion_empty_input() {
        let mut results = vec![];
        hyperfusion_rank(&mut results, "test", QueryIntent::Factual, 0.5, &HashMap::new());
    ...
    fn all_identical_scores_use_half() {
        let scores = vec![
            RawScores { bm25: 0.5, semantic: 0.5, temporal: 0.5, authority: 0.5, evidence: 0.5, diversity: 0.5, depth: 0.5, consensus: 0.5 },
            RawScores { bm25: 0.5, semantic: 0.5, temporal: 0.5, authority: 0.5, evidence: 0.5, diversity: 0.5, depth: 0.5, consensus: 0.5 },
        ];
    ...
    fn min_max_normalize_correct() {
        let scores = vec![
            RawScores { bm25: 0.0, semantic: 0.0, temporal: 0.0, authority: 0.0, evidence: 0.0, diversity: 0.0, depth: 0.0, consensus: 0.0 },
            RawScores { bm25: 1.0, semantic: 1.0, temporal: 1.0, authority: 1.0, evidence: 1.0, diversity: 1.0, depth: 1.0, consensus: 1.0 },
        ];
        let normalized = min_max_normalize(scores);
    ...
    fn detect_intent_works() {
        ...
        assert_eq!(detect_intent("Rust vs Go performance"), QueryIntent::Comparison);
        assert_eq!(detect_intent("arxiv paper transformer attention"), QueryIntent::Academic);
        assert_eq!(detect_intent("github rust async code"), QueryIntent::Code);
        assert_eq!(detect_intent("Rust latest news 2026"), QueryIntent::CurrentEvents);
    }
    ...
    fn hyperfusion_populates_scores() {
        ...
        results[0].rank = 1;
        results[1].rank = 2;
        hyperfusion_rank(&mut results, "systems", QueryIntent::Factual, 0.5, &HashMap::new());
