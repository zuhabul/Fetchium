...
// ─── Model Registry ──────────────────────────────────────────────────────────
//
// **Single source of truth for all model names in HyperSearchX.**
...
impl ModelRegistry {
    ...
    pub fn fast_model(kind: ProviderKind) -> &'static str {
        match kind {
            ProviderKind::GeminiCli => "gemini-3-flash-preview",
        ...
        match kind {
            ProviderKind::GeminiCli => &[
                ...
                ModelInfo {
                    ...
                    aliases: &["flash25", "2.5flash", "flash2.5"],
                    note: "Gemini 2.5 Flash — stable, well-tested fallback",
                },
                ModelInfo {
            ...
            ProviderKind::Gemini => &[
                ModelInfo {
                    ...
                    note: "Gemini 2.5 Flash — recommended via REST API (default)",
                },
                ModelInfo {
                    id: "gemini-2.5-pro",
                    capability: ModelCapability::Powerful,
                ...
                ModelInfo {
                    ...
                    note: "Gemini 2.0 Flash — legacy (retiring June 2026)",
                },
            ],
...
impl ProvidersConfig {
    ...
    pub fn resolved_chain(&self) -> Vec<ProviderKind> {
        ...
        // GeminiCli is included because `gemini auth login` tokens work for the
        // subprocess even when they lack sufficient scopes for the REST API.
        //
        // NOTE: Anthropic is only auto-added when a real API key is present.
        // The Claude Code OAuth token is NOT sufficient — api.anthropic.com
        // returns "OAuth authentication is currently not supported" (401).
        ...
        let has_anthropic_key = self
            ...
            .is_some();
        let auto_fallbacks: [(ProviderKind, bool); 4] = [
...
mod tests {
    ...
    fn resolve_model_unknown_alias_passes_through() {
        let mut entry = ProviderEntry::default();
        entry.model = Some("my-custom-model:latest".into());
        assert_eq!(
