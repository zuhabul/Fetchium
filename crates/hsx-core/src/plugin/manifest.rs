...
impl PluginManifest {
    ...
    pub fn load(path: &std::path::Path) -> Result<Self, HsxError> {
        ...
        toml::from_str(&content)
            .map_err(|e| HsxError::Config(format!("plugin.toml parse error: {e}")))
    }
    ...
    pub fn scaffold(name: &str, plugin_type: &str, path: &std::path::Path) -> Result<(), HsxError> {
        let manifest = PluginManifest {
            ...
            description: format!("A HyperSearchX {plugin_type} plugin"),
            author: None,
            homepage: None,
            config_schema: None,
        };
        let content = toml::to_string_pretty(&manifest)
            .map_err(|e| HsxError::Config(format!("serialise error: {e}")))?;
