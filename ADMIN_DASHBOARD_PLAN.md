        ...
        headers: [
          ...
          { key: 'X-Robots-Tag', value: 'noindex, nofollow, noarchive' },
          {
            ...
            value: [
              ...
              "connect-src 'self' wss://admin.fetchium.com https://api.fetchium.com",
              "font-src 'self'",
              "frame-src 'none'",
              "object-src 'none'",
              "base-uri 'self'",
...
FETCHIUM_INTERNAL_API_URL=https://api.fetchium.com
FETCHIUM_ADMIN_SESSION_SECRET=         # For admin session signing
  ...
  providers: [
    Credentials({
      async authorize(credentials) {
        ...
        const res = await fetch(`${process.env.FETCHIUM_INTERNAL_API_URL}/internal/admin/auth/login`, {
          method: 'POST',
...
services:
  admin:
    ...
    environment:
      ...
      FETCHIUM_INTERNAL_API_URL: http://api:3050
      ADMIN_DB_PATH: /data/admin.db
    volumes:
      - admin-data:/data
...
- **Environment secrets**: NEXTAUTH_SECRET_ADMIN, FETCHIUM_INTERNAL_API_URL
...
| `FETCHIUM_INTERNAL_API_URL` | Internal API base URL | ✓ |
